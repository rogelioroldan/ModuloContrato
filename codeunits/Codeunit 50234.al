codeunit 50234 "Sales Post via Contrato Queue"
{
    TableNo = "Contrato Queue Entry";
    ...
    local procedure SetJobQueueStatus(var SalesHeader: Record "Sales Header"; NewStatus: Enum "Document Job Queue Status"; JobQueueEntry: Record "Contrato Queue Entry")
    begin
    ...
    var
        JobQueueEntry: Record "Contrato Queue Entry";
    begin
    ...
    local procedure FillJobEntryFromSalesSetup(var JobQueueEntry: Record "Contrato Queue Entry")
    var
        SalesSetup: Record "Sales & Receivables Setup";
    begin
        SalesSetup.Get();
        JobQueueEntry."Notify On Success" := SalesSetup."Notify On Success";
        JobQueueEntry."Contrato Queue Category Code" := GetJobQueueCategoryCode();
    end;

    local procedure FillJobEntrySalesDescription(var JobQueueEntry: Record "Contrato Queue Entry"; SalesHeader: Record "Sales Header")
    begin
    ...
    local procedure AreOtherJobQueueEntriesScheduled(JobQueueEntry: Record "Contrato Queue Entry"): Boolean
    var
        SalesReceivablesSetup: Record "Sales & Receivables Setup";
        JobQueueEntryFilter: Record "Contrato Queue Entry";
        result: Boolean;
    begin
        SalesReceivablesSetup.Get();
        JobQueueEntryFilter.SetFilter("Contrato Queue Category Code", GetJobQueueCategoryCode());
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeSetJobQueueStatus(SalesHeader: Record "Sales Header"; NewJobQueueStatus: Option " ","Scheduled for Posting",Error,Posting; JobQueueEntry: Record "Contrato Queue Entry")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnEnqueueJobEntryOnBeforeEnqueue(SalesHeader: Record "Sales Header"; var JobQueueEntry: Record "Contrato Queue Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeBatchProcessingErrorReset(var JobQueueEntry: Record "Contrato Queue Entry"; var IsHandled: Boolean)
    begin
    end;
}
