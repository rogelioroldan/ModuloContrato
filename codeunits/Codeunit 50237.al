codeunit 50237 "Sales Post Invoice Contrato"
{
    ...
    var
        ...
        ContratoSalesLine: Record "Sales Line";
        GLEntryNo: Integer;
        LineCount: Integer;
    begin
        SalesHeader := DocumentHeaderVar;
        ...
        if TempInvoicePostingBuffer.Find('+') then
            repeat
                ...
                then begin
                    SetContratoLineFilters(ContratoSalesLine, TempInvoicePostingBuffer);
                    ContratoPostLine.PostContratoSalesLines(ContratoSalesLine.GetView(), GLEntryNo);
                end;
    ...
    procedure PrepareContratoLine(SalesHeaderVar: Variant; SalesLineVar: Variant; SalesLineACYVar: Variant)
    var
        SalesHeader: Record "Sales Header";
        SalesLine: Record "Sales Line";
        SalesLineACY: Record "Sales Line";
    ...
    local procedure SetContratoLineFilters(var ContratoSalesLine: Record "Sales Line"; InvoicePostingBuffer: Record "Invoice Posting Buffer")
    begin
        ContratoSalesLine.Reset();
        ContratoSalesLine.SetRange("Job No.", InvoicePostingBuffer."Job No.");
        ContratoSalesLine.SetRange("No.", InvoicePostingBuffer."G/L Account");
        ContratoSalesLine.SetRange("Gen. Bus. Posting Group", InvoicePostingBuffer."Gen. Bus. Posting Group");
        ContratoSalesLine.SetRange("Gen. Prod. Posting Group", InvoicePostingBuffer."Gen. Prod. Posting Group");
        ContratoSalesLine.SetRange("VAT Bus. Posting Group", InvoicePostingBuffer."VAT Bus. Posting Group");
        ContratoSalesLine.SetRange("VAT Prod. Posting Group", InvoicePostingBuffer."VAT Prod. Posting Group");
        ContratoSalesLine.SetRange("Dimension Set ID", InvoicePostingBuffer."Dimension Set ID");

        if InvoicePostingBuffer."Fixed Asset Line No." <> 0 then begin
            SalesSetup.Get();
            if SalesSetup."Copy Line Descr. to G/L Entry" then
                ContratoSalesLine.SetRange("Line No.", InvoicePostingBuffer."Fixed Asset Line No.");
        end;

        SalesPostInvoiceEvents.RunOnAfterSetJobLineFilters(ContratoSalesLine, InvoicePostingBuffer);
    end;
