...
table 50235 "Contrato Queue Log Entry"
{
    Caption = 'Contrato Queue Log Entry';
    ReplicateData = false;
    ...
    fields
    {
        ...
        field(17; "Contrato Queue Category Code"; Code[10])
        {
    ...
    var
        ...
        ConfirmDeletingEntriesQst: Label 'Are you sure that you want to delete Contrato queue log entries?';
    ...
    begin
        ...
        OnBeforeMarkAsError(Rec, ContratoQueueEntry, ErrorMessage);

        if ContratoQueueEntry.Get(ID) then
            ContratoQueueEntry.SetError(ErrorMessage);
    ...
    local procedure OnBeforeDeleteEntries(var ContratoQueueLogEntry: Record "Contrato Queue Log Entry"; var SkipConfirm: Boolean);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeMarkAsError(var ContratoQueueLogEntry: Record "Contrato Queue Log Entry"; var ContratoQueueEntry: Record "Contrato Queue Entry"; var ErrorMessage: Text)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnDeleteEntriesOnBeforeDeleteAll(var ContratoQueueLogEntry: Record "Contrato Queue Log Entry");
    begin
    end;
